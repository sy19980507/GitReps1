...
public class ARIMA {
	...
	public double[] preDeal()
	{
		...
		{
			/*System.out.println(originalData[i+7]);
	...
	public int[] getARIMAmodel()
	{
 	...
 	public List<Double> getListARIMAmodel(){
		List<Double> allPredictData = new ArrayList<>();
		double[] stdoriginalData=this.preDeal();//ԭʼ���ݱ�׼������
		int paraType=0;
		double minAIC=9999999;
		int bestModelindex=0;
		...
		try {
			for(int i=0;i<model.length;i++)
			{
				...
				double temp=getmodelAIC(armaARMAcoe,stdoriginalData,paraType);
				temp = formatDouble(temp);
				System.out.println(temp);
				allPredictData.add(temp);
				if (temp<minAIC)
				{
					bestModelindex=i;
					minAIC=temp;
			...
			return allPredictData;
		}catch (Exception e){
			e.printStackTrace();
		}
		//System.out.println("bestValue"+allPredictData.get(bestModelindex));
		return allPredictData;
	}
